(() => {
  ...
  function play(song){
    ...
    if(match){const frame=node('iframe');frame.src='https://suno.com/embed/'+match[1];frame.title=(song.title||'Song')+' player';frame.allow='autoplay; encrypted-media; fullscreen';frame.referrerPolicy='strict-origin-when-cross-origin';frame.setAttribute('sandbox','allow-scripts allow-same-origin');dialog.append(frame);}
    ...
    if(song.lyrics){const details=node('details');details.append(node('summary','Lyrics'),node('pre',song.lyrics));dialog.append(details);}
