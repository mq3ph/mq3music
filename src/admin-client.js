...
cats=['NAME SONGS','INSPIRATIONAL SONGS','OPM','ORIGINAL SONGS'];
...
async function apiDelete(path){
  ...
  if(!r.ok){
    ...
    throw Error(data.error||'Delete failed.');
  }
...
function badge(value){
  return node('span',value,'badge '+value);
}
...
async function load(){
  [songs,requests,orders]=await Promise.all([
    api('/api/admin/songs'),
...
async function enter(){
  ...
  $('setup').textContent=
    ...
    Object.entries(session.setup)
      .map(([k,v])=>`${k}: ${v?'configured':'needs setup'}`)
      .join(' · ');
...
for(const name of ['Name Request',...cats,'GCash','PayPal']){
  const b=button(name,()=>{
    tab=name;
    $('admin-search').value='';
    render();
...
async function deleteRequest(r){
  if(!confirm(`Delete the name request from ${r.name} (${r.email})?`)){
    return;
  }

  await apiDelete(`/api/admin/requests/${r.id}`);

  message(`Request from ${r.name} deleted.`);
  await load();
}
...
function render(){
  ...
  [...$('tabs').children].forEach(b=>{
    ...
    b.setAttribute(
      'aria-pressed',
      String(b.textContent===tab)
    );
  });

  $('new-song').classList.toggle(
    'hidden',
    !cats.includes(tab)
  );
  ...
  $('summary').textContent=
    `${songs.length} songs · `+
    `${requests.filter(r=>r.status==='pending').length} pending name requests · `+
    `${paid.length} verified payments · `+
    `${money(paid.reduce((n,o)=>n+o.amount,0))} verified revenue`;
  ...
  if(cats.includes(tab)){
    ...
    songs
      .filter(s=>s.category===tab&&match(s))
      .forEach(s=>
        row(body,[
          s.title,
          s.names,
          money(s.price),
          s.lyrics?'Yes':'No',
          s.audio_path?'Uploaded':'Missing',
          s.preview_path?'Uploaded':'None',
          badge(s.published?'published':'draft'),
          button(
            'Edit / Upload',
            ()=>editSong(s)
          )
    ...
    orders
      ...
      .forEach(o=>
        row(body,[
          ...
          o.kind==='membership'
            ?'Membership'
            :songs.find(s=>s.id===o.song_id)?.title||'Song',
          money(o.amount),
          o.reference,
          badge(o.status),
          date(o.expires_at),
...
function editSong(s={}){
  ...
  $('song-category').value=s.category||tab;
  $('song-names').value=s.names||'';
  $('song-lyrics').value=s.lyrics||'';
  $('song-price').value=(s.price||0)/100;
  $('song-published').checked=!!s.published;
...
$('song-form').onsubmit=async e=>{
  ...
  try{
    ...
    const files=[
      ['audio',$('full-file').files[0]],
      ['preview',$('preview-file').files[0]]
    ];
    ...
    const data={
      id:$('song-id').value||undefined,
      title:$('song-title').value,
      category:$('song-category').value,
      names:$('song-names').value,
      lyrics:$('song-lyrics').value,
      price:Math.round(
        Number($('song-price').value)*100
      ),
      published:false
    };
    ...
    await api(
      ...
      {
        ...data,
        published:$('song-published').checked
      }
    );
...
function editRequest(r){
  ...
  songs
    ...
    .forEach(s=>{
      ...
      $('request-song').append(o);
    });

  $('request-song').value=
    r.song_id||'';

  $('request-editor').showModal();
}

$('cancel-request').onclick=()=>
  $('request-editor').close();
...
$('backup').onclick=()=>{
  ...
  a.href=url;
  a.download='mq3-private-metadata-backup.json';
...
enter().catch(e=>{
  if(
    !/Log in|Session expired/.test(e.message)
  ){
    message(e.message);
  }
});
