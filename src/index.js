...
export function createApp(s=services()){
  ...
  const sameOrigin=req=>{
    if(req.get('Origin')!==origin()){
      fail(403,'This action must come from the MQ3 app.');
    }
  };

  async function admin(req){
    const v=req.cookies.mq3_admin;

    if(!v||!/^[a-f0-9]{64}$/.test(v)){
      fail(401,'Log in to your admin dashboard.');
    }
    ...
    if(!rows.length){
      fail(401,'Session expired. Log in again.');
    }
  }

  async function limit(req,scope,max){
    const ip=env.VERCEL
      ?(req.get('x-vercel-forwarded-for')||req.socket.remoteAddress)
      :req.socket.remoteAddress;

    const key=hash(
      `${scope}:${ip||'unknown'}:${Math.floor(Date.now()/600000)}`
    );

    const rows=await q(
      'INSERT INTO limits(key,hits,expires_at) VALUES($1,1,$2) ON CONFLICT(key) DO UPDATE SET hits=limits.hits+1 RETURNING hits',
      [key,new Date(Date.now()+1200000)]
    );
  ...
  function customer(req,res){
    let value=req.cookies.mq3_customer;
  ...
  async function owned(req,id){
    const [o]=await q(
      ...
      [
        uuid(id),
        hash(req.cookies.mq3_customer||'none')
      ]
    );

    if(!o){
      fail(404,'Order not found in this browser.');
    }
  ...
  app.post('/api/login',wrap(async(req,res)=>{
    ...
    if(!env.ADMIN_PASSWORD_HASH){
      fail(503,'Set up your admin password first.');
    }

    await limit(req,'login',8);

    if(!verifyPassword(req.body.password,env.ADMIN_PASSWORD_HASH)){
  ...
  app.post('/api/logout',wrap(async(req,res)=>{
    ...
    if(req.cookies.mq3_admin){
      await q(
        'DELETE FROM sessions WHERE token_hash=$1',
        [hash(req.cookies.mq3_admin)]
      );
    }
  ...
  app.get('/api/catalog',wrap(async(req,res)=>{
    ...
    res.json({
      songs:songs.map(t=>({
        ...t,
        addedAt:new Date(t.created_at).getTime()
      })),
      payments:{
        gcash:!!env.GCASH_NUMBER,
        paypal:!!env.PAYPAL_PAYMENT_URL
      },
      membership:{
        price:Math.round(
          Number(env.MEMBERSHIP_PRICE_PHP||199)*100
        ),
        days:Number(env.MEMBERSHIP_DAYS||30)
      },
      contact:env.OWNER_EMAIL||''
    });
  }));

  app.post('/api/requests',wrap(async(req,res)=>{
    sameOrigin(req);
    await limit(req,'request',6);
    ...
    res.status(201).json({
      ok:true,
      message:'Your request is recorded. MQ3 can email you when the song is available.'
    });
  }));

  app.get('/api/admin/session',wrap(async(_req,res)=>{
    res.json({
      ok:true,
      setup:{
        blob:!!env.BLOB_READ_WRITE_TOKEN&&!!env.BLOB_HOST,
  ...
  app.get('/api/admin/songs',wrap(async(_req,res)=>{
    res.json(
      await q(
  ...
  app.post('/api/admin/songs',wrap(async(req,res)=>{
    ...
    if(
      ...
      fail(400,'Check category and price.');
    }

    const names=String(req.body.names||'').slice(0,500);
    const lyrics=String(req.body.lyrics||'').slice(0,30000);
    ...
    if(req.body.id){
      ...
      if(req.body.published&&!existing.audio_path){
        fail(400,'Upload full audio before publishing.');
      }

      await q(
        'UPDATE songs SET title=$2,category=$3,names=$4,lyrics=$5,price=$6,published=$7 WHERE id=$1',
        [
          id,
          title,
          category,
      ...
      await q(
        'INSERT INTO songs(id,title,category,names,lyrics,price) VALUES($1,$2,$3,$4,$5,$6)',
        [
          id,
          title,
          category,
  ...
  app.post('/api/admin/upload-ticket',wrap(async(req,res)=>{
    if(!env.BLOB_READ_WRITE_TOKEN||!env.BLOB_HOST){
      fail(503,'Connect your private Vercel Blob store first.');
    }

    const songId=uuid(req.body.songId);
    const kind=req.body.kind;

    if(!['audio','preview'].includes(kind)){
      fail(400,'Invalid upload type.');
    }
    ...
    if(
      ...
      fail(404,'Save the song first.');
    }

    const id=randomUUID();
    const pathname=`songs/${songId}/${kind}-${id}.mp3`;
  ...
  async function finishUpload(ticket,blob){
    ...
    if(!t){
      fail(400,'Upload ticket not found.');
    }

    if(blob.pathname!==t.pathname){
      fail(400,'Upload path does not match ticket.');
    }
    ...
    const result=await s.upload({
      ...
      onBeforeGenerateToken:async(pathname,clientPayload)=>{
        ...
        if(!t||pathname!==t.pathname){
          fail(400,'Upload ticket expired or invalid.');
        }
      ...
      onUploadCompleted:async({blob,tokenPayload})=>
        finishUpload(tokenPayload,blob)
    });
  ...
  app.post('/api/admin/upload-finish',wrap(async(req,res)=>{
    const [t]=await q(
      'SELECT * FROM upload_tickets WHERE id=$1',
      [uuid(req.body.ticket)]
    );

    if(!t){
      fail(404,'Upload ticket not found.');
    }

    const response=await s.audio(
      t.pathname,
      'bytes=0-2'
    );
  ...
  app.get('/api/admin/requests',wrap(async(_req,res)=>{
    res.json(
      await q(
  ...
  app.delete('/api/admin/requests/:id',wrap(async(req,res)=>{
  ...
  app.post('/api/admin/requests/:id',wrap(async(req,res)=>{
    ...
    if(!['pending','working','available'].includes(status)){
      fail(400,'Invalid request status.');
    }

    const songId=req.body.songId
      ?uuid(req.body.songId)
      :null;

    if(status==='available'&&!songId){
      fail(
        400,
...
Music. Quality. 3rd Gen.`,
  ...
  app.post('/api/orders',wrap(async(req,res)=>{
    ...
    if(
      !['gcash','paypal'].includes(provider)||
      !['song','membership'].includes(kind)
    ){
      fail(400,'Invalid payment selection.');
    }

    if(
      provider==='gcash'&&!env.GCASH_NUMBER||
      provider==='paypal'&&!env.PAYPAL_PAYMENT_URL
    ){
      fail(
        503,
      ...
      instructions:
        provider==='gcash'
          ?{
            ...
            account:env.GCASH_ACCOUNT_NAME||''
          }
          :{
            url:env.PAYPAL_PAYMENT_URL
          },
      message:'After paying, submit your transaction reference. Access starts only after MQ3 verifies the payment.'
    });
  }));

  app.post('/api/orders/:id/reference',wrap(async(req,res)=>{
    sameOrigin(req);
    await limit(req,'reference',12);
  ...
  app.get('/api/my-orders',wrap(async(req,res)=>{
    if(!req.cookies.mq3_customer){
      return res.json([]);
    }
  ...
  app.get('/api/admin/orders',wrap(async(_req,res)=>{
    res.json(
      await q(
  ...
  app.post('/api/admin/orders/:id/review',wrap(async(req,res)=>{
    ...
    if(
      !o||
      !['submitted','paid'].includes(o.status)
    ){
      fail(
        409,
...
Keep this link private. Memberships expire on the stated access date.`,
  ...
  app.get('/api/songs/:id/audio',wrap(async(req,res)=>{
    ...
    if(
      ![200,206,416].includes(response.status)
    ){
      fail(
        502,
