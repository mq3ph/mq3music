...
export function mp3RequestRoutes({app,env,query:q,sameOrigin,limit}){
 ...
 app.post('/api/account/mp3-requests',wrap(async(req,res)=>{
   ...
   if(!result){const [duplicate]=await q('SELECT * FROM mp3_requests WHERE user_id=$1 AND song_id=$2',[user.id,songId]);if(duplicate)return res.json({ok:true,alreadyRequested:true,request:duplicate});fail(400,'Not enough Credits for this request. Load Credits, then try again.');}
