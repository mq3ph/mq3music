...
(async()=>{const browser=await chromium.launch({executablePath:'C:/Program Files/Google/Chrome/Application/chrome.exe',headless:true});try{
 ...
 const songs=[{id:'suno1',title:'Moses',category:'NAME SONGS',lyrics:'Line one\n<script>unsafe</script>',suno_url:'https://suno.com/song/657e28fc-df67-4dd1-be2d-24bfaa952503',gifts_enabled:true},{id:'mp3',title:'Existing MP3',category:'NAME SONGS'}];
 ...
 assert.equal(await page.locator('#mq3-suno-dialog').getByRole('button',{name:'Send a gift',exact:true}).count(),1);
 assert.equal(await page.locator('#mq3-suno-dialog').getByRole('link',{name:'Listen on Suno',exact:true}).count(),0);
 assert.equal(await page.locator('#mq3-suno-dialog').getByText('Press Play in the Suno player below.',{exact:true}).count(),0);
 await page.locator('#mq3-suno-dialog').getByRole('button',{name:'Send a gift',exact:true}).click();await page.locator('#mq3-gift-dialog[open]').waitFor();await page.evaluate(()=>document.querySelector('#mq3-gift-dialog').close());
 ...
 await page.evaluate(()=>{tracks[0].gifts_enabled=false;openSunoSong(tracks[0]);});assert.equal(await page.locator('#mq3-suno-dialog').getByRole('button',{name:'Send a gift',exact:true}).count(),0);await page.getByRole('button',{name:'Close player',exact:true}).click();await page.waitForFunction(()=>!document.querySelector('#mq3-suno-dialog iframe'));
